...
sodium_matches = glob.glob(os.path.join(ARG1, "*_alignedtoRef.nii.gz"))
...
matches = glob.glob(os.path.join(ARG1, f"{ARG2}*_alignedtoRef.nii.gz"))
if not matches:
    raise FileNotFoundError(f"No reference sodium ({ARG2}*_alignedtoRef.nii.gz) found in {ARG1}")

ref_sodium = sorted(matches)[0]
print(f"✅ Using reference sodium file: {ref_sodium}")

# Get all matching TSCs for this reference
ref_base = strip_ext(ref_sodium)
tsc_matches = glob.glob(os.path.join(ARG1, f"{ARG2}_TSC*_alignedtoRef.nii.gz"))
if not tsc_matches:
    print("ℹ️ No TSC files found for reference sodium.")
    ref_sodium_tsc_list = []
else:
    ref_sodium_tsc_list = sorted(tsc_matches)
    print(f"✅ Found {len(ref_sodium_tsc_list)} TSC files for reference sodium.")
...
# --- Register reference sodium to MPRAGE ---
...
# --- Apply same transform to ALL sodium/TSC alignedtoRef files ---
for s in sodium_matches + ref_sodium_tsc_list:
    out_file = f"{strip_ext(s)}_toMPRAGE.nii.gz"
    if os.path.exists(out_file):
        print(f"⏭️ Skipping {os.path.basename(out_file)} (already aligned).")
        continue
    run([
        f"{FSLDIR}/bin/flirt",
        "-in", s,
...
all_sodium_in_mprage = [
    f"{strip_ext(s)}_toMPRAGE.nii.gz"
    for s in sodium_matches + ref_sodium_tsc_list
    if os.path.exists(f"{strip_ext(s)}_toMPRAGE.nii.gz")
]
...
tsc_files = [
    ...
    and "bet" not in f
    and (
        ("_alignedtoRef" in f or "resampled" in f)
